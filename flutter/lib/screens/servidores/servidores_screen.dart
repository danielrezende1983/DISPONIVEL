...
import 'package:evento_gerente/screens/servidores/componentes/servidor_list_tile.dart';
...
class ServidoresScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<ServidorManager>(
        builder: (_, servidorManager, __){

          final listaServidor = servidorManager.listaFiltrada();

          return ListView.builder(
              padding: const EdgeInsets.all(4),
              itemCount: listaServidor.length,
              itemBuilder: (_, index){
                return ServidorListTile(listaServidor[index]);
              }
          );
